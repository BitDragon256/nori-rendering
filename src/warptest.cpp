...
static const std::string kWarpTypeNames[WarpTypeCount] = {
    "square", "triangle", "disk", "uniform_sphere", "uniform_hemisphere",
...
struct WarpTest {
    ...
    void generatePoints(int &pointCount, PointType pointType,
        ...
        // special handling for QMC pointsets
        if (pointType == Halton || pointType == HaltonDecorrelated)
        {
            pointCount = sqrtVal*sqrtVal;
            PropertyList propList;
            propList.setInteger("sampleCount", pointCount);

            unique_ptr<Sampler> qmcSampler;
            if (pointType == Halton)
                qmcSampler = std::unique_ptr<Sampler>(static_cast<Sampler*>(NoriObjectFactory::createInstance("halton", propList)));
            else if (pointType == HaltonDecorrelated)
                qmcSampler = std::unique_ptr<Sampler>(static_cast<Sampler*>(NoriObjectFactory::createInstance("halton_decorr", propList)));

            qmcSampler->generate();

            //precompute and sort all samples for nice coloring
            std::vector<Point2f> qmcPoints(static_cast<size_t>(pointCount));
            std::generate(qmcPoints.begin(), qmcPoints.end(), [&qmcSampler]() -> Point2f {Point2f next = qmcSampler->next2D(); qmcSampler->advance(); return next;});
            std::sort(qmcPoints.begin(), qmcPoints.end(), [](const Point2f& a, const Point2f& b) -> bool {return a.y() < b.y() || (a.y() == b.y() && a.x() < b.x());});

            positions.resize(3, pointCount);
            weights.resize(1, pointCount);

            for (int i=0; i<pointCount; ++i) {
                const Point2f& sample = qmcPoints[i];
        ...
        for (int i=0; i<pointCount; ++i) {
            ...
            switch (pointType) {
                ...
                case Stratified:
                    ...
                    break;
                case Halton:
                case HaltonDecorrelated:
                    throw std::logic_error("QMC handlers should have already been handled by now.");
            }
    ...
    create_microfacet_bsdf(float alpha, float kd, float bsdfAngle) {
        ...
        auto * brdf = (BSDF *) NoriObjectFactory::createInstance("roughconductor", list);
...
public:
    ...
    void refresh() {
        ...
        try {
            ...
            // QMC pointsets may not be implemented yet
            if (pointType > Stratified)
                m_pointTypeBox->set_selected_index(0);
            else
    ...
    void draw_contents() {
        ...
        if (m_drawHistogram) {
            ...
            const int histHeight = (warpType == Square || warpType == Triangle || warpType == Disk) ? histWidth : histWidth / 2;
    ...
    void initializeGUI() {
        ...
        m_pointTypeBox = new ComboBox(m_window, { "Independent", "Grid", "Stratified", "Halton", "Halton (decorrelated)" });
        m_pointTypeBox->set_callback([&](int) { refresh(); });

        new Label(m_window, "Warping method", "sans-bold");
        m_warpTypeBox = new ComboBox(m_window, { "Square", "Triangle", "Disk", "Sphere", "Hemisphere (unif.)",
