...
protected:
    ...
    uint32_t findMesh(uint32_t &idx) const {
        ...
        if (m_meshOffset.size() > 64) //threshold estimated. needs benchmark to set the ideal value
            it = std::lower_bound(m_meshOffset.begin(), m_meshOffset.end(), idx+1) - 1;
        //linear search is more predictable and therefore faster, especially when there are only a few meshes :)
        else
            it = std::find_if(m_meshOffset.begin(), m_meshOffset.end(), [=](uint32_t i){return i > idx;}) - 1;
        idx -= *it;
